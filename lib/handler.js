...
function onEnd ( req, func, callback ) {
    ...
    req.on ( 'data', function ( chunk ) {
		...
		if ( rawDataIsJSON ) {
	        ...
	        callback ( resStructure ( 400, false, err.message ) );
		}
    } ).on ( 'error', function ( err ) {
        callback ( resStructure ( 200, false, err.message ) );
    } );    
}
...
exports.updateOneByMap = function ( req, model, map, callback, testSessionData ) {
    var func = function ( rawData ) { 
        model.find ( { where: map } )
            .success ( function ( raw ) {
                raw.updateAttributes ( JSON.parse ( rawData ) )
        ...
        model.findAll ( { where: map } )
            .success ( function ( raws ) {
                for ( var i = 0 ; i < raws.length ; i++ ) {
                    raws[i].updateAttributes ( JSON.parse ( rawData ) )
                         .success ( function ( res ) {
               			callback ( resStructure ( 200, true, 'Record has been updated.' ) );
                    } )
                    .error ( function ( err ) {
