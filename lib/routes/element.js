exports.route = function ( req, crud, model, callback ) {
    if ( req.method === 'GET' ) {
        var occ;
        if ( occ = req.url.match ( /^\/element\/(\d+)/ ) ) {
            crud.get ( model, occ[1], callback );
        }  else if ( occ = req.url.match ( /^\/element\/(\w+)/)) {
            crud.find ( model, 'ele_name', occ[1], callback );
        } else if ( req.url.match ( /^\/elements/ ) ) {
        ...
        var occ;
        if ( occ = req.url.match ( /^\/element\/([0-9]+)/ ) ) {
